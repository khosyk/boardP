...
class GameContainer extends Component {
  ...
  getFirstData = async () => {
    try {
      const result = await axios.get(`${config.host}/posts?page=1&type=game`);
  ...
  getCurrentPage = async (e) => {
    ...
    try {
      const result = await axios.get(
        `${config.host}/posts?page=${currentPage}type=game`
      );
      const {
        data: { contents: contentsData, page },
      } = result;
  ...
  getSearch = async () => {
    try {
      const result = await axios.get(`${config.host}/posts?type=game`);
  ...
  render() {
    ...
    const data = this.props.contents.sort(function test(a,b){ return a-b});
