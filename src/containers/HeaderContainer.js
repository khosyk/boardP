...
import { setLogin, logout } from "../modules/login";
...
const Header = ({ location: { pathname } }) => {
	...
	async function handleCookie() {
		try {
			const cookie = cookies.USID;
			if (!cookie) {
				Object.assign(userInfo, { userId: "", userActive: false });
				return;
			}
			const url = "http://119.196.222.239:4000/users/token-verify";
	...
	// 마운트된 후 한번만 호출
	useEffect(() => {
		console.log("ComponentDidMount", test1);
	...
	return (
		<>
			<Test user={user} setLogin={setLogin} test1={test1} />
			<Main src={uthis} />
